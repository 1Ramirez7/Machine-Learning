...
# ok this is steps to adding the mini test data frmae

ok first i need to add the mini holdup manually 

```{python}
# this steps are not needed in main since it is jsut to update  my county dataset
import pandas as pd
# note this id column was name ' id ' so a space before and after id so need to check that for hold out
...
# Save the updated DataFrame as a new CSV file
df1.to_csv("king_housing-filterstep5.csv", index=False)
...
df2 = pd.read_csv("C://git//ml//housing_project//testcode//king_housing-filterstep5.csv")

# now combing major and minor but adding 'm' in between to cut out more duplicates
df2['more'] = df2['Major'].astype(str) + 'm' + df2['Minor'].astype(str)

# Save the updated DataFrame to a new CSV file
df2.to_csv("C://git//ml//housing_project//testcode//king_housing-filterstep6.csv", index=False)

print("File saved successfully as 'king_housing-filterstep6.csv'")
...
# Save the updated DataFrame to a new CSV file
df2.to_csv("C://git//ml//housing_project//testcode//merged_data_5.csv", index=False)

print("File saved successfully as 'merged_data_5.csv'")
...
This next steps filters merged_data_5.csv to to only have values where more